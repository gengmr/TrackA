...
# model_location = "models_pretrained/dped_srcnn"

from unet_v1 import unet as test_model                  # import your model definition as "test_model"
model_location = "models_pretrained/iphone_iteration_80300.ckpt"         # specify the location of your saved pre-trained model (ckpt file)

compute_PSNR_SSIM = True
compute_running_time = True

if __name__ == "__main__":
    record_psnr, record_ssim = 0, 0
    ...
    if compute_PSNR_SSIM:
        ...
        with tf.Session(config=config) as sess:
            ...
            ssim_score = 0.0
            psnr_score = 0.0
            validation_images = os.listdir("/root/Desktop/valid/patches/canon/")
            num_val_images = len(validation_images)

            for j in range(num_val_images):

                image_phone = misc.imread("/root/Desktop/valid/patches/iphone/" + validation_images[j])
                image_dslr = misc.imread("/root/Desktop/valid/patches/canon/" + validation_images[j])
            ...
            record_psnr, record_ssim = psnr_score, ssim_score
            print("\n-------------------------------------\n")
            # sess.close()
    ...
    if compute_running_time:
        ...
        avg_time_baseline, max_ram = utils.compute_running_time("superres", "models_pretrained/dped_srcnn.pb", "/root/Desktop/valid/HD_res/")
        ...
        print('---------------------------final score-------------------------')
        print ('RAM : ' + str(max_ram/1024) + 'GB')
        print('mean PSNR : ' + str(record_psnr))
        print('mean SSIM : ' + str(record_ssim))
        print ('mean speed up:' + str(float(avg_time_baseline) / avg_time_solution))
        print('score A: ' + str(
            4 * (record_psnr - 21) + 100 * (record_ssim - 0.9) + 2 * min(avg_time_baseline / avg_time_solution, 4)))
        print('score B: ' + str(
            1 * (record_psnr - 21) + 400 * (record_ssim - 0.9) + 2 * min(avg_time_baseline / avg_time_solution, 4)))
        print('score C: ' + str(
            2 * (record_psnr - 21) + 200 * (record_ssim - 0.9) + 2.9 * min(avg_time_baseline / avg_time_solution, 4)))
        print('-----------------------------------------------------------------------')
    ...
    #-----------------------performance---------------------------------------
        ...
        with tf.Session(config=None) as sess:
            ...
            for j in range(num_val_images):
                ...
                [enhanced] = sess.run([out_], feed_dict={x_: image_phone, y_: image_dslr})
                compare_img = np.concatenate((image_phone, enhanced, image_dslr), axis=1)[0, :, :, :]
                misc.imsave('output/patch' + validation_images[j], compare_img)
